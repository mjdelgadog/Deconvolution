...
// @authors : D.Guffanti, M.Delgado S.Manthey
// @created : Jan 26, 2022 
// Gauss & Wiener Filters
//========================================================= 
    ...
    public:
      ...
      struct CmplxWaveform_t {
        ...
        inline CmplxWaveform_t operator*(const CmplxWaveform_t& cwf) const {
          ...
          for (int i = 0; i<(int)fCmplx.size(); i++) {
...
namespace opdet {
  ...
  void Deconvolution::SourceSPEDigiDataFile() {
    ...
    if ((int)fDigiDataColumn >= n_columns) {
  ...
  Double_t Deconvolution::ComputeNormalization(CmplxWaveform_t& xGH) {
    ...
    for (size_t  k=ileft; k<= (size_t)iright; k++) norm += x[k]; 
