...
namespace opdet {
  class Deconvolution : public art::EDProducer{
    ...
    private:
      ...
      TVirtualFFT* fft_r2c; 
      TVirtualFFT* fft_c2r; 
      TVirtualFFT* fftextra_r2c; 
      TVirtualFFT* fftextra_c2r; 
      int  fExtraSamples;
...
namespace opdet {
  ...
  {  
    ...
    fftextra_r2c = TVirtualFFT::FFT(1, &fExtraSamples, "M R2C K"); 
    fftextra_c2r = TVirtualFFT::FFT(1, &fExtraSamples, "M C2R K"); 
  ...
  void Deconvolution::produce(art::Event& evt){
    ...
    double firstWaveformTime = 0;                                                    //     
    for (auto const& wf : digi_wave)                                                 //
       {                                                                             //
         if (firstWaveformTime < 0) firstWaveformTime = wf.TimeStamp();              // 
       }                                                                             //                                                                                                                            
    //************Temporary*************//********************************************
    ...
    for (auto const& wf: digi_wave) {
      ...
      for (int i=0; i<fSamples*0.5+1; i++) {
        ...
        //if (i==0) {
          //printf("SPE_Max = %g -> S2 = %g\n", SPE_Max, S2); 
        //}

        if (fApplyPrefilter) {
          Double_t prefilter_PSD = fxG0.fCmplx.at(i).Rho2(); 
          N2 *= prefilter_PSD;
        }
      ...
      if (fApplyPrefilter) {xV = (fxG0 * xV); xV.MakeReAndIm();}
      xY  = xG * xV;
      xY.MakeReAndIm();
      ...
      if (fApplyPostBLCorr) {
        ...
        //if (fFilterConfig.fType == EFilterType::kWiener) {
          //decPedestal *= 0.5; 
        //}
      }
    ...
    //******************************************************************************************
    //-- Create histograms ----Temporary
    //******************************************************************************************
        
     for (size_t i=0; i<out_recob->size(); i++) {
        // Create a new histogram
      const auto rwf = out_wave->at(i);
      const auto dwf = out_recob->at(i); 
      
      // Make a name for the histogram
      
      std::stringstream name;
      
      //="decowf_"+std::to_string(WfDeco)+"_"+std::to_string(dwf.Channel())+ "_"+std::to_string(rwf.TimeStamp());
      
      name << "event_"      << evt.id().event() 
              << "_opchannel_" << std::to_string(dwf.Channel())
              << "_decowaveform_"  << mapChannelWF[dwf.Channel()];
      
      mapChannelWF[dwf.Channel()]++ ;
       
      double starTime = rwf.TimeStamp()-firstWaveformTime;
      double endTime = double(dwf.Signal().size())/fSampleFreq + starTime;
      
      TH1F * hs_wiener = tfs->make< TH1F >(name.str().c_str(), TString::Format (";t - %f (#mus);",firstWaveformTime), NOpDetWaveform, starTime, endTime);
      
      for(size_t tick=0; tick<dwf.Signal().size(); tick++){
        hs_wiener->SetBinContent(tick+1, dwf.Signal()[tick] );  //or Fill
      }
      hs_wiener->Draw();
    }
