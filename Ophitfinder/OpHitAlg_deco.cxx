...
namespace opdet {
                    ...
                    bool use_start_time)
  {
    ...
    for (int i=0; i< int (opWaveformVector.size()); i++){
      ...
      // Get the result
      auto const& pulses = threshAlg.GetPulses();
      for (auto const& pulse : pulses) ConstructHit(opDetWaveformVector,hitThreshold,channel,timeStamp,pulse,hitVector,clocksData,calibrator,use_start_time);
  }
}
  
  //----------------------------------------------------------------------------
  void ConstructHit(std::vector<raw::OpDetWaveform> const&,float hitThreshold,int channel,double timeStamp,pmtana::pulse_param const& pulse,std::vector<recob::OpHit>& hitVector,detinfo::DetectorClocksData const& clocksData,calib::IPhotonCalibrator const& calibrator,bool use_start_time){
