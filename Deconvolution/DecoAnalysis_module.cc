...
namespace opdet {
  ...
  void DecoAnalysis::analyze(const art::Event& evt){ 
    ...
    evt.getByLabel(fInputModule, fInstanceName, deconv);
    
    // art::Handle< std::vector< raw::OpDetWaveform >> deconv;
    // evt.getByLabel(fInputModule, fInstanceName, deconv);
    ...
    for (auto const& decowaveform : *deconv){
      if (firstWaveformTime < 0) firstWaveformTime = double(decowaveform.TimeStamp().GetTimeStamp());
    }
    ...
    for (auto const& decowaveform : *deconv){
      ...
      double endTime = double(decowaveform.NSignal())/fSampleFreq + startTime;
      ...
      // std::cout << " decowaveform " << decowaveform.size() << std::endl; 
      for(size_t tick = 0; tick < decowaveform.NSignal(); tick++){
        ...
        // std::cout <<  decowaveform[tick] << std::endl;
      }                  
      //std::cout << " event " << evt.id().event() << " OpChannel " << channel << " BinContent # " << BinContent << " binnum " << binNum << std::endl;     
    }                                     
  }
  //  std::cout << " MaxAmplitud " << Maxamplitud << " OpChannel " << channel << std::endl;     
} // namespace opdet
